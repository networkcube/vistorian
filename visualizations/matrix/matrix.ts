...
enum UpdateOptions {
    ...
    PlotLocation = Nodes | Geometry | Links | 1 << 4,
...
function updateNodes() {
    ...
    labelsLeft.enter().append('text')
        ...
        .attr('text-anchor', 'end')
        .attr('x', plotMargin.left - 10)
        .attr('y', (d, i) => { return plotMargin.top + cellsize * nodeOrder[d.id()] + cellsize; })
        .on('mouseover', (d, i) => {
            networkcube.highlight('set', <networkcube.ElementCompound>{ nodes: [d] });
        })
        .on('mouseout', (d, i) => {
    ...
    labelsTop.enter().append('text')
        ...
        .attr('class', 'labelsTop nodeLabel')
        .text((d, i) => { return d.label(); })
        .attr('x', (d, i) => { return plotMargin.left + cellsize * nodeOrder[d.id()] + cellsize; })
        .attr('y', plotMargin.left - 10)
        .attr('transform', (d, i) => { return 'rotate(-90, ' + (plotMargin.top + cellsize * i + cellsize) + ', ' + (plotMargin.left - 10) + ')' })
        .on('mouseover', (d, i) => {
            networkcube.highlight('set', <networkcube.ElementCompound>{ nodes: [d] });
    ...
    labelsTop
        ...
        .attr('transform', (d, i) => { return 'rotate(-90, ' + (plotMargin.top + cellsize * nodeOrder[d.id()] + 12) + ', ' + (plotMargin.left - 10) + ')' })
