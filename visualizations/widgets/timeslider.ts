...
class TimeSlider {
    ...
    appendTo(svg: D3.Selection, x?: number, y?: number) {
        ...
        // // create tickmarks
        // var times = this.dgraph.times().toArray();
        // // console.log('times', times)
        // var num_ticks = Math.floor(this.sliderWidth / this.TICK_GAP);
        // var gran_curr = this.dgraph.gran_min;
        // var tickTimes: networkcube.Time[] = times;
        // if (tickTimes.length <= num_ticks) {
        //     this.drawTickmarks(gran_curr, tickTimes, g);
        // }
        // while (tickTimes.length > 0 && gran_curr <= this.dgraph.gran_max) {
        //     gran_curr++;
        //     tickTimes = [];
